...
import { UsersService } from '@/services/local/UserStorageService';
...
/*
const handleTokenRefresh = async (): Promise<string | null> => {
  const auth = UsersService.getCurrentUser();
  ...
  try {
    ...
    UsersService.authenticate({
      ...
      tokens: {
        ...auth.tokens,
        access_token: newAccessToken,
        refresh_token: response.data.refresh_token || auth.tokens.refresh_token,
      },
    });
...
const handleTokenRefresh = async (): Promise<string> => {
  ...
  UsersService.authenticate(updatedAuth);
