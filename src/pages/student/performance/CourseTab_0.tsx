...
//               <tr>
//                 <th>Assessments</th>
//                 <th className="d-none d-md-table-cell">Home Work</th>
...
const LessonAssessmentTable: React.FC<LessonAssessmentTableProps> = ({ title, icon, scores }) => {
  // Group scores by lesson (scope_title)
  const grouped = scores.reduce((acc: Record<string, any[]>, score: any) => {
  ...
  // Collect all unique assessment types across all lessons
  const allTypes = Array.from(new Set(scores.map((s: any) => s.type))).sort();
      ...
      <div className="card-body p-0">
        <div className="table-responsive">
          <table className="table table-hover mb-0">
            ...
            <tbody>
              {Object.entries(grouped).map(([lessonName, lessonScores]: [string, any[]]) => {
                // Get the lesson date from the first score (all should be same lesson)
