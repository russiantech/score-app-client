...
//   console.log(`Tutors: ${JSON.stringify(tutors)}`);
...
const AssignTutors: React.FC = () => {
  ...
  const fetchInitialData = async () => {
    try {
      ...
      // UserService.getAll returns the full api_response wrapper:
      // { success, message, data: { users: User[], page_meta: {...} } }
      // or may return a plain array for some callers — handle both.
      const tutorsRaw = tutorsResponse as any;
      const tutorsList: User[] =
        Array.isArray(tutorsRaw)
          ? tutorsRaw
          : Array.isArray(tutorsRaw?.data?.users)
          ? tutorsRaw.data.users
          : Array.isArray(tutorsRaw?.data)
          ? tutorsRaw.data
          : [];
      setTutors(tutorsList);

      // CourseService.getAll has the same wrapper shape.
      const coursesRaw = coursesResponse as any;
      const coursesList: Course[] =
        Array.isArray(coursesRaw)
          ? coursesRaw
          : Array.isArray(coursesRaw?.data?.courses)
          ? coursesRaw.data.courses
          : Array.isArray(coursesRaw?.data)
          ? coursesRaw.data
          : [];
      setCourses(coursesList);

      const statsData = statsResponse?.data ?? (statsResponse as any)?.success ?? null;
      setStats(statsData);
    } catch (error) {
      console.error('Failed to fetch initial data:', error);
      toast.error('Failed to load initial data');
