...
// import { FormSelect, FormInput } from '@/components/forms/Forms';
...
const fetchInitialData = async () => {
  try {
    ...
    /* 
      The longer-term fix would be updating UserService.getAll() and CourseService.getAll() return types 
      to reflect the actual { data: { users: User[], ... } } shape — 
      but as any unblocks the build immediately without touching the service layer.
    */
    setStudents((studentsResponse as any).data?.users || []);
    setCourses((coursesResponse as any).data?.courses || []);
    setStats(statsResponse.data || null);
    /*  */
  } catch (error) {
    console.error('Failed to fetch initial data:', error);
                    ...
                    {enrollments.map(enrollment => {
                      ...
                      return (
                        <div key={enrollment.id} className="border-bottom p-3">
                          <div className="d-flex justify-content-between align-items-start mb-2">
                            <div className="d-flex align-items-center">
                              <StudentAvatar names={student.names || 'NA'} size={40} />
                              <div className="ms-3">
                                <div className="fw-bold">{student.names || 'Unknown'}</div>
                                <div className="small text-muted">{student.email || 'No email'}</div>
                              </div>
                          ...
                          {enrollments.map(enrollment => {
                            ...
                            return (
                              <tr key={enrollment.id}>
                                <td className="ps-4">
                                  <div className="d-flex align-items-center">
                                    <StudentAvatar names={student.names || 'NA'} />
                                    <div className="ms-3">
                                      <div className="fw-bold">{student.names || 'Unknown'}</div>
                                      <div className="text-muted small">{student.email || 'No email'}</div>
                                    </div>
              ...
              <div className="card-body">
                {students.length === 0 ? (
                  ...
                  <div className="row g-3">
                    {students.map(student => {
                      const studentEnrollments = getCoursesForStudent(student.id);
                      // const studentEnrollments = getCoursesForStudent(student.id) as Partial<Enrollment[]>;
                      return (
                        <div className="col-12 col-sm-6 col-lg-6 col-xl-6" key={student.id}>
                          <div className="border rounded p-3 h-100 bg-light">
                            <div className="d-flex align-items-center mb-3">
                              <StudentAvatar names={student.names || ''} size={40} bgColor="bg-primary" />
                              <div className="ms-3 flex-grow-1 overflow-hidden">
                                <div className="fw-bold text-truncate">{student.names}</div>
                                <div className="text-muted small text-truncate">{student.email}</div>
                              </div>
                            ...
                            {studentEnrollments.length === 0 ? (
                              ...
                              <>
                                ...
                                <ul className="list-unstyled small mb-0">
                                  {studentEnrollments.map(e => {
                                    // const course = e.course || {};
                                    const course = (e.course || {}) as Partial<Course>;
                                    return (
                                      <li
                                        key={e.id}
                                        className="d-flex justify-content-between align-items-center mb-2 pb-2 border-bottom"
